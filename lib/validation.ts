...
import { ProjectDetails, SignInFormValues,  ForgetFormValues, AddAuthorsValues  } from "@/types/index";
...
export const addAuthorValidate = (values: any) => {  
    let errors: FormikErrors<AddAuthorsValues>  = {}; 
     if (!values.email) {
    errors.email = 'Required';
} else if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(values.email)) {
    errors.email = 'Invalid email address';
